...
tf.app.flags.DEFINE_float(
    'C_param',0.1,'penalty paramter of the error term.')
tf.app.flags.DEFINE_float(
    'delta',1.0, 'The parameter set for margin.')
...
def loss_fn(W,b,x_data,y_target):
    ...
    #hinle loss
    classification_loss = tf.reduce_mean(tf.maximum(0.,
                        tf.subtract(FLAGS.delta,tf.multiply(logits,y_target))))
    total_loss = tf.add(tf.multiply(FLAGS.C_param,classification_loss),
...
def next_batch_fn(x_train,y_train,num_sample = FLAGS.batch_size):
    index = np.random.choice(len(x_train),size = num_sample)
...
for step_idx in range(FLAGS.num_steps):
    ...
    X_batch,y_batch = next_batch_fn(x_train,y_train,num_sample=FLAGS.batch_size)
